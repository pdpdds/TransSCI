// FotoSCIhop.cpp : Defines the entry point for the application.
//
...
BOOL DoFileOpen(HWND hwnd, char *filename, char *ext)
{
   ...
   if(proceed)
   {
	  ...
	  if (!stricmp((ext==NULL?szFileName+ofn.nFileExtension:ext), "v56"))
		...
		isPicture=true;
		//TODO cambiare e prevedere il caso che non sia nessuno dei due
      ...
      if (result==ID_NOERROR)
	  {
		  ...
		  {
			int pos = 2;
			for (unsigned int i = strlen(szFileName); i>strlen(szFileName)-maxstrlen+3; i--) 
				if (szFileName[i] == '\\')
					pos = i;
...
BOOL DoFileSaveAs(HWND hwnd)
{
   ...
   ofn.Flags = OFN_EXPLORER | OFN_PATHMUSTEXIST | OFN_HIDEREADONLY |
               OFN_OVERWRITEPROMPT;
    //TODO check questa parte qui in poi!!!     
...
BOOL DoFileExport(HWND hwnd)
{
   ...
   if(GetSaveFileName(&ofn))
   {
		...
		if (tempfile)
		{
			...
			//eventualmente modificare il 256
...
BOOL DoFileImport(HWND hwnd)
{
   ...
   if(GetOpenFileName(&ofn))
   {
		...
		if (tempfile)
		{
			...
			//i seguenti sono facoltativi a scelta dell'utente:
	...
	/*		{			
			...
			if (dwremainder)
				expectedsize+= newheight*(4-dwremainder); //bmp vuole il DWORD align per ogni scanline
			...
			if (expectedsize>imsize)//(expectedsize!=imsize)      mettiamo minore per photoshop
			{
			...
			if (remap)
			{
				...
				//TODO implementare il REMAPPING!!!!!!
...
void DoPropertyBox(HWND hwnd)
{
	...
	if (isPicture)
	{
		...
		sprintf(propstr, INTERFACE_V56PROPSTR,
						' ',' ',
...
/*
   hWndTopBar = CreateWindowEx(WS_EX_STATICEDGE, "STATIC", "  SCIMunito Immagina 0.95 by Enrico Rolfi", 
            WS_CHILD | WS_VISIBLE | SS_LEFTNOWORDWRAP, //SS_ETCHEDHORZ,
            //0, 0, 575, 20, hWnd, NULL, GetModuleHandle(NULL), NULL);
			-2, 0, 600, 20, hWnd, NULL, GetModuleHandle(NULL), NULL);

   SetWindowFont(hWndTopBar, hfDefault, false);*/
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		//SelectBrush(hdc, GetSysColorBrush(COLOR_BTNFACE));//cambiare colore!!
		SetBkMode(hdc, TRANSPARENT);
		//Rectangle(hdc, 0,0,600,20);
		GetWindowRect(hWnd, &rc);
		...
		if (curCell)
			{
				...
				//oppure TextOut(hdc, 230,0, finalstr,strlen(finalstr));
		...
		if (curShownImage)
		{	
			//mettiamo qui pure la palette per ora in base a se esiste l'immagine
			if (picX>0)
			{
