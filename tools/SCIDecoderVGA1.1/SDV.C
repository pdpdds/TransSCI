...

{
  ...
  if (resnum != lastresnum) { /* don't close a file only to open it again */
    ...
    if (!ismessage)  //drigo
      ...
      sprintf(resname, "%s\\resource.msg", gamedir);
  ...
  if (data == NULL) {
    ...
    textcolor(4);
    sprintf(err, "Not enough memory. %d", datainfo[3]);
    message(err/*"Not enough memory.%d"*/);  //drigo
  ...
  switch (datainfo[4]) {
    ...
    //case 1:      //Drigo not true case 1 is used in PQ1rem
    case 2:
...

{
  ...
  if (readmap() == NULL) {
    ...
    printf("Could not find RESOURCE.MAP or MESSAGE.MAP");  //Drigo
  ...
  boxprint(30, 2, 80, 24,
      ...
      "     SCI DECODER/VGA v. 1.1\n\n"

      "(c) 2003, Modified by Enrico Rolfi\n"

      "Original Program by Carl Muckenhoupt\n"
      ...
      "D to dump selection to unencoded file\n"

      "X to dump all data to unencoded files\n"
  ...
  cputs("Thank you for using SCI DECODER!\r\n\n");
